...
- Explain WHY the change was made, this is more important than WHAT was changed.
...
2. Commit with body:
   ...
   ```bash
   git commit -m "feat(auth): implement two-factor authentication for better security
...
3. Commit with resolved issues:
   ...
   ```bash
   git commit -m "docs: add additional troubleshooting steps for arm64 architecture which occurs on macos
